...
const TOURNAMENT_NAME_KEY = "ligue 1";
        ...
        {isAdmin && (
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button
              ...
              style={{
                background: pending.length > 0 ? "rgba(0,191,255,0.15)" : "rgba(255,255,255,0.05)",
                border: `1px solid ${pending.length > 0 ? "rgba(0,191,255,0.5)" : "rgba(255,255,255,0.15)"}`,
                color: pending.length > 0 ? "#00BFFF" : "rgba(255,255,255,0.5)",
            ...
            >
              ⏳ PENDING RESULTS
              {pending.length > 0 && (
                <span style={{ background: "#00BFFF", color: "#000", borderRadius: "50%", width: 22, height: 22, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.75rem", fontWeight: 700, fontFamily: "inherit" }}>
                  {pending.length}
                </span>
              )}
            </button>
      ...
      <div style={{ padding: "0 20px 40px" }}>
        {loading || tabLoading ? <LoadingSpinner /> : (
          <>
            ...
            {tab === "fixtures" && <FixturesList tournamentName="Ligue 1" />}
            {tab === "results" && (
              <>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
                  <button
                    onClick={() => isAdmin ? setEditResult(null) : setSubmitOpen(true)}
                    style={{ background: "linear-gradient(135deg, #00BFFF, #87CEEB)", border: "none", color: "#000", padding: "14px 32px", borderRadius: 30, fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.4rem", letterSpacing: 2, cursor: "pointer", boxShadow: "0 4px 20px rgba(0,191,255,0.4)" }}
                  >
                    + ADD RESULT
                  </button>
                </div>
