        ...
        setClubs(prev => {
          ...
          for (const seasonData of Object.values(seasons)) {
            ...
            for (const entry of Object.values(table)) {
              const name = (entry?.name || entry?.team || "").trim();
              if (name && !existing.has(name.toLowerCase())) {
                existing.add(name.toLowerCase());
                extras.push({ id: name, name, badge: entry?.badge || entry?.icon || "" });
              }
            }
          }
          if (!extras.length) return prev;
          return [...prev, ...extras].sort((a, b) => (a.name || "").localeCompare(b.name || ""));
        });
      })
    );

    return () => { unsub1(); unsub2(); unsub3(); unsub4(); leagueUnsubs.forEach(u => u()); };
  }, [isAdmin]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ padding: "24px 20px 60px" }}>
        ...
        {/* Loading spinner */}
        {(calData === null || activeMonths === null) && (
        ...
        {calData !== null && activeMonths !== null && (activeMonths).length === 0 && (
        ...
        {/* Month cards */}
        {calData !== null && activeMonths !== null && [...activeMonths]
          .sort((a, b) => a.year !== b.year ? a.year - b.year : a.month - b.month)
          .map(({ year, month }, mi) => {
      ...
      <AddFixturesModal
        ...
        teamIconRegistry={teamIconRegistry}
        clubs={clubs}
        showToast={showToast}
      />
