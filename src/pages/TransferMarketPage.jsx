...
function EnterAuctionCard({ manager }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setDone(true);
      setTimeout(() => { setOpen(false); setDone(false); setPlayerName(""); setClub(manager?.team || ""); setStartingBid(""); }, 1400);
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }} onClick={() => setOpen(false)}>
          <div style={{ ...GLASS, borderRadius: "24px", padding: "36px", maxWidth: "480px", width: "100%", position: "relative" }} onClick={e => e.stopPropagation()}>
            ...
            {done ? (
              ...
              <>
                {[
                  { label: "Player Name", value: playerName, setter: setPlayerName, placeholder: "e.g. Erling Haaland" },
                  { label: "Club", value: club, setter: setClub, placeholder: "e.g. Manchester City" },
                  { label: "Starting Bid (€)", value: startingBid, setter: setStartingBid, placeholder: "e.g. 50000000", type: "number" },
                  ...
                  <div key={label} style={{ marginBottom: "18px" }}>
                    ...
                    <input value={value} onChange={e => setter(e.target.value)} placeholder={placeholder} type={type || "text"} style={{ ...inputStyle, width: "100%", boxSizing: "border-box", padding: "14px 18px", fontSize: "1rem" }} />
                  </div>
                ))}
                <div style={{ marginBottom: "18px" }}>
                  ...
                  <div style={{ ...inputStyle, width: "100%", boxSizing: "border-box", padding: "14px 18px", fontSize: "1rem", opacity: 0.6 }}>{creatorName}</div>
                </div>
...
function AuctionGridCard({ player, teamIcons, onClick, isAdmin, onDelete, auctionDeadline }) {
  ...
  return (
    <div style={{ background: isSold ? "rgba(0,255,136,0.06)" : "rgba(255,255,255,0.04)", border: isSold ? "1px solid rgba(0,255,136,0.4)" : "1px solid rgba(255,20,147,0.18)", borderRadius: "20px", overflow: "hidden", cursor: "pointer", transition: "all 0.25s", display: "flex", flexDirection: "column", position: "relative" }}
      ...
      {isSold && (
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 5, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", borderRadius: "20px" }}>
          ...
          {player.soldTo && <div style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.9rem", marginTop: "8px", textAlign: "center", padding: "0 12px" }}>{player.soldTo}</div>}
        </div>
      )}
      <div style={{ width: "100%", aspectRatio: "1/1", background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
        ...
        {isExpired && !isSold && <div style={{ position: "absolute", top: "10px", left: "10px", background: "rgba(255,68,68,0.9)", borderRadius: "8px", padding: "4px 10px", color: "#fff", fontSize: "0.7rem", fontWeight: 700 }}>EXPIRED</div>}
      </div>
      <div style={{ padding: "16px", flex: 1, display: "flex", flexDirection: "column", gap: "8px" }}>
        ...
        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.85rem" }}>👥 Interested: <span style={{ color: "#fff", fontWeight: 700 }}>{bids.length}</span></div>
        {highestBid > 0 && <div style={{ color: "#00ff88", fontSize: "0.85rem", fontWeight: 700 }}>Leading: {formatMoney(highestBid)}</div>}
      ...
      {isAdmin && <button onClick={e => { e.stopPropagation(); onDelete(); }} style={{ position: "absolute", top: "8px", right: "8px", background: "rgba(255,0,0,0.8)", border: "none", borderRadius: "8px", color: "#fff", fontWeight: 700, fontSize: "0.8rem", padding: "4px 8px", cursor: "pointer", zIndex: 10 }}>🗑️</button>}
    </div>
  );
}
  ...
  async function handleBid() {
    ...
    try {
      await push(ref(db, `${PATHS.transfers}/auction/${player.id}/bids`), {
        amount: amt,
        club: manager.team,
        managerName: manager.username,   // ← correct field from Firebase
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div style={{ width: "160px", height: "160px", margin: "0 auto 20px", borderRadius: "50%", overflow: "hidden", background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", border: "3px solid rgba(255,20,147,0.4)" }}>
        {player.imageUrl ? <img src={player.imageUrl} alt={player.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "85%", height: "85%" }}><ShirtSVG clubName={player.club} playerName={player.name} squadNumber={player.squadNumber} /></div>}
      </div>
      ...
      {auctionDeadline && timeLeft && (
        <div style={{ ...GLASS, borderRadius: "16px", padding: "20px", marginBottom: "24px", textAlign: "center" }}>
          <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "12px" }}>{isExpired ? "⏰ AUCTION ENDED" : "⏳ TIME REMAINING"}</div>
          {isExpired ? (
      ...
      {!isExpired && manager && !player.sold && (
        <div style={{ marginBottom: "20px" }}>
          <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "8px", fontWeight: 700 }}>Place Your Bid — Min: {formatMoney(minBid)}</div>
          <div style={{ ...GLASS, borderRadius: "14px", padding: "16px", marginBottom: "12px" }}>
            ...
            <div style={{ color: "#fff", fontWeight: 700 }}>{manager.username} · {manager.team}</div>
          </div>
          <input value={bidAmount} onChange={e => setBidAmount(e.target.value)} placeholder={`Min ${formatMoney(minBid)}`} type="number" style={{ ...inputStyle, width: "100%", boxSizing: "border-box", padding: "14px 18px", fontSize: "1rem", marginBottom: "12px" }} />
...
function NegotiationCard({ offer, isOwn, isAdmin, manager }) {
  ...
  async function handleAccept() {
    ...
    try {
      ...
      if (offer.type === "buy") {
        const snap = await get(ref(db, `career_team_management/${lendingClub}/squad`));
        const data = snap.val();
        if (data) {
          for (const [key, p] of Object.entries(data)) {
            if (p.name === playerName) {
              await remove(ref(db, `career_team_management/${lendingClub}/squad/${key}`));
              const { loanStatus, loanClub, loanFrom, ...clean } = p;
              await push(ref(db, `career_team_management/${borrowingClub}/squad`), clean);
              break;
            }
          }
        }
        ...
        if (data) {
          for (const [key, p] of Object.entries(data)) {
            if (p.name === playerName) {
              ...
              const { loanStatus, loanClub, loanFrom, ...clean } = p;
              await push(ref(db, `career_team_management/${borrowingClub}/squad`), { ...clean, role: "reserve", loanStatus: "in", loanFrom: lendingClub });
              break;
            }
          }
        }
  ...
  async function handleCancel() {
    setProcessing(true); setActionError("");
    try {
      await update(ref(db, `${PATHS.transfers}/negotiations/${offer.id}`), { status: "cancelled" });
    } catch (e) { setActionError("Failed: " + e.message); }
    setProcessing(false);
  }

  // ── REVERSE: undo financials, move player back ────────────────────────
  async function handleReverse() {
    ...
    try {
      ...
      // Reverse finance: refund buying club, deduct from selling club
      if (buyingClub && amt > 0) {
        await push(ref(db, `career_team_management/${buyingClub}/finance/transactions`), {
          type: "income", category: "Player Sales",
          source: `[REVERSED] ${offer.playerName}`, amount: amt,
          month: monthName, monthIndex, year, createdAt: Date.now(),
          reversedNegotiationId: offer.id,
        });
      }
      if (sellingClub && amt > 0) {
        await push(ref(db, `career_team_management/${sellingClub}/finance/transactions`), {
      ...
      // Reverse squad: move player back
      const playerName = offer.playerName;
      if (offer.type === "buy") {
        // Find in buying club, move back to selling club
        const buySnap = await get(ref(db, `career_team_management/${buyingClub}/squad`));
        const buyData = buySnap.val();
        if (buyData) {
          for (const [key, p] of Object.entries(buyData)) {
            if (p.name === playerName) {
              await remove(ref(db, `career_team_management/${buyingClub}/squad/${key}`));
              const { loanStatus, loanClub, loanFrom, ...clean } = p;
              await push(ref(db, `career_team_management/${sellingClub}/squad`), clean);
              break;
            }
          }
        }
      } else if (offer.type === "loan") {
        // Remove from borrowing club, restore in lending club
        const buySnap = await get(ref(db, `career_team_management/${buyingClub}/squad`));
        const buyData = buySnap.val();
        if (buyData) {
          for (const [key, p] of Object.entries(buyData)) {
            if (p.name === playerName && p.loanStatus === "in") {
              await remove(ref(db, `career_team_management/${buyingClub}/squad/${key}`));
              break;
            }
          }
        }
        const sellSnap = await get(ref(db, `career_team_management/${sellingClub}/squad`));
        const sellData = sellSnap.val();
        if (sellData) {
          for (const [key, p] of Object.entries(sellData)) {
            if (p.name === playerName && p.loanStatus === "out") {
              await update(ref(db, `career_team_management/${sellingClub}/squad/${key}`), { loanStatus: null, loanClub: null, role: p.originalRole || "first-team" });
              break;
            }
          }
        }
  ...
  return (
    <div style={{ padding: "36px 40px", background: isOwn ? "rgba(255,20,147,0.1)" : "rgba(255,255,255,0.03)", border: `1px solid ${isOwn ? "rgba(255,20,147,0.4)" : "rgba(255,255,255,0.08)"}`, borderRadius: "20px", marginBottom: "20px" }}>
      ...
      {/* Admin-only: Cancel (any status) or Reverse (accepted only) */}
      {isAdmin && (
        <div style={{ display: "flex", gap: "12px", marginTop: "16px" }}>
          {!["cancelled", "reversed"].includes(offer.status) && (
            <button onClick={handleCancel} disabled={processing} style={{ flex: 1, padding: "14px", background: "rgba(170,170,170,0.15)", border: "1px solid rgba(170,170,170,0.4)", borderRadius: "12px", color: "#aaa", fontWeight: 700, fontSize: "1.1rem", cursor: processing ? "not-allowed" : "pointer" }}>
              🚫 Cancel
            </button>
          )}
          {isAccepted && (
            <button onClick={() => setShowReverseConfirm(true)} disabled={processing} style={{ flex: 1, padding: "14px", background: "rgba(170,68,255,0.15)", border: "1px solid rgba(170,68,255,0.5)", borderRadius: "12px", color: "#aa44ff", fontWeight: 700, fontSize: "1.1rem", cursor: processing ? "not-allowed" : "pointer" }}>
              ↩️ Reverse
            </button>
          )}
        </div>
      ...
      {showReverseConfirm && (
        <div style={{ marginTop: "16px", background: "rgba(170,68,255,0.08)", border: "1px solid rgba(170,68,255,0.3)", borderRadius: "14px", padding: "20px" }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.95rem", marginBottom: "16px" }}>
            This will refund <strong style={{ color: "#fff" }}>{offer.fromClub}</strong> and deduct from <strong style={{ color: "#fff" }}>{offer.toClub || offer.playerClub}</strong>, and move {offer.playerName} back.
          </div>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={handleReverse} disabled={processing} style={{ flex: 1, padding: "14px", background: "#aa44ff", border: "none", borderRadius: "12px", color: "#fff", fontWeight: 700, cursor: processing ? "not-allowed" : "pointer" }}>
              {processing ? "Reversing..." : "✅ Confirm Reverse"}
            </button>
            <button onClick={() => setShowReverseConfirm(false)} style={{ flex: 1, padding: "14px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "12px", color: "#fff", cursor: "pointer" }}>
              Cancel
            </button>
          </div>
        </div>
...
export default function TransferMarketPage() {
  ...
  // Auto-process auction wins when deadline expires (admin only)
  useEffect(() => {
    ...
    const checkExpiry = async () => {
      ...
      for (const player of auctionPlayers) {
        ...
        try {
          ...
          if (buyingClub && amt > 0) {
            await push(ref(db, `career_team_management/${buyingClub}/finance/transactions`), { type: "expense", category: "Player Purchase", source: player.name, amount: amt, month: monthName, monthIndex, year, createdAt: Date.now() });
          }
          if (sellingClub && amt > 0) {
            await push(ref(db, `career_team_management/${sellingClub}/finance/transactions`), { type: "income", category: "Player Sales", source: player.name, amount: amt, month: monthName, monthIndex, year, createdAt: Date.now() });
          }
          const snap = await get(ref(db, `career_team_management/${sellingClub}/squad`));
          const data = snap.val();
          if (data) {
            for (const [key, p] of Object.entries(data)) {
              if (p.name === player.name) {
                ...
                const { loanStatus, loanClub, loanFrom, ...clean } = p;
                await push(ref(db, `career_team_management/${buyingClub}/squad`), clean);
                break;
              }
            }
          }
  ...
  // Negotiations sort: own → accepted → latest pending → rejected → cancelled → reversed
    ...
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'Inter', sans-serif", position: "relative" }}>
      ...
      {headlineVideo ? (
        <div style={{ position: "relative", width: "100%", aspectRatio: "16/7", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", bottom: "20px", left: "20px", color: "#FF1493", fontFamily: "'Bebas Neue', sans-serif", fontSize: "2rem", letterSpacing: "3px", textShadow: "0 2px 12px rgba(0,0,0,0.8)" }}>💸 Transfer Window</div>
        </div>
      ) : (
      ...
      <Modal active={!!selectedAuction} onClose={() => setSelectedAuction(null)} wide>
        {selectedAuction && <AuctionPopup player={selectedAuction} manager={manager} isAdmin={isAdmin} auctionDeadline={auctionDeadline} teamIcons={mergedIcons} onClose={() => setSelectedAuction(null)} />}
      </Modal>
