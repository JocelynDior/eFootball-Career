...
function posBarColor(pos) {
  if (pos <= 8)  return "#3b82f6"; // blue  → R16
  if (pos <= 24) return "#f97316"; // orange → play-offs
  return "#ef4444";                // red → eliminated
}

function getTeamIcon(cache, teamName) {
  const url = cache?.[teamName];
  if (!url) return null;
  return <img src={url} alt={teamName} style={{ width: 36, height: 36, objectFit: "contain", borderRadius: 4 }} />;
}
...
function LeaguePhaseTab({ teamIconsCache }) {
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto" }}>
        ...
        <thead>
          <tr style={{ background: "rgba(255,20,147,0.15)" }}>
            {["Pos", "Club", "P", "W", "D", "L", "GF", "GA", "GD", "Pts"].map(h => (
              <th key={h} style={{ padding: "14px 12px", color: "rgba(255,255,255,0.6)", fontWeight: 700, fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "1px", textAlign: h === "Club" ? "left" : "center", whiteSpace: "nowrap", borderBottom: "1px solid rgba(255,20,147,0.2)" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
              ...
              >
                ...
                <td style={{ padding: "14px 12px", textAlign: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}>
                    <div style={{ width: 4, height: 28, background: barColor, borderRadius: 2, flexShrink: 0 }} />
                    <span style={{ color: "#fff", fontWeight: 700, fontSize: "1rem" }}>{pos}</span>
                  </div>
                </td>
                {/* Club */}
                <td style={{ padding: "14px 12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {getTeamIcon(teamIconsCache, team.name)}
                    <span style={{ color: "#fff", fontWeight: 700, fontSize: "1rem", whiteSpace: "nowrap" }}>{team.name}</span>
                  </div>
                </td>
                {[team.p||0, team.w||0, team.d||0, team.l||0, team.gf||0, team.ga||0,
                  ((team.gd||0) > 0 ? `+${team.gd}` : team.gd||0)].map((v, ci) => (
                  <td key={ci} style={{ padding: "14px 12px", textAlign: "center", color: "rgba(255,255,255,0.8)", fontSize: "0.95rem" }}>{v}</td>
                ))}
                {/* Pts */}
                <td style={{ padding: "14px 12px", textAlign: "center" }}>
                  <span style={{ color: "#FF1493", fontWeight: 900, fontSize: "1.1rem" }}>{team.pts || 0}</span>
                </td>
              </tr>
            );
          })}
      ...
      {/* Legend */}
      <div style={{ display: "flex", gap: 24, flexWrap: "wrap", padding: "18px 0 4px" }}>
        {[
          { color: "#3b82f6", label: "Round of 16 (Top 8)" },
          { color: "#f97316", label: "Play-offs (9th–24th)" },
...
function FixturesTab({ teamIconsCache }) {
    ...
    const unsub = onValue(ref(db, FIXTURES_ROOT), snap => {
      ...
      for (const [dateStr, dateData] of Object.entries(data)) {
        ...
        for (const tourn of Object.values(dateData.tournaments)) {
          ...
          const name = tourn.name.trim().toLowerCase().replace(/\s/g, "");
          if (name !== "tokyoleague" && name !== "tokyo") continue;
  ...
  if (!allFixtures.length) return (
    <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,0.3)" }}>
      ...
      <div style={{ fontSize: "0.9rem", marginTop: 8 }}>Add Tokyo League fixtures in the Calendar page.</div>
    </div>
  );
...
export default function TokyoPage() {
  ...
  const adminMenuItems = [
    { icon: "📜", label: "Results History", action: () => { setHistoryOpen(true); setPlusOpen(false); } },
    { icon: "🎞️", label: "Manage Slideshow", action: () => { setSlideshowOpen(true); setPlusOpen(false); } },
    { icon: "🏆", label: "Bracket Images", action: () => { setBracketOpen(true); setPlusOpen(false); } },
    { icon: "🎯", label: "Set Active Results Tab", action: () => { setActiveTabOpen(true); setPlusOpen(false); } },
    { icon: "📋", label: "Edit League Table", action: () => { setTableEditOpen(true); setPlusOpen(false); } },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'Inter', sans-serif", position: "relative" }}>
      ...
      {/* Navbar with admin + button injected via extraActions */}
      <Navbar
          ...
          <div ref={plusRef} style={{ position: "relative" }}>
            ...
            {plusOpen && (
              <div style={{
                ...
                {adminMenuItems.map(({ icon, label, action }) => (
      ...
      <div style={{ padding: "28px 24px 80px", maxWidth: "100%" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(2.4rem, 7vw, 4rem)", letterSpacing: "6px", color: "#FF1493", margin: 0, textShadow: "0 0 30px rgba(255,20,147,0.5)" }}>{LEAGUE_NAME}</h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", marginTop: 8 }}>30 Teams · Season {SEASON}</p>
          <div style={{ marginTop: 10, display: "inline-block", background: "rgba(255,20,147,0.1)", border: "1px solid rgba(255,20,147,0.3)", borderRadius: 20, padding: "4px 16px", color: "rgba(255,255,255,0.55)", fontSize: "0.8rem" }}>
            Active: <span style={{ color: "#FF1493", fontWeight: 700 }}>{RESULT_SUB_TABS.find(s => s.id === activeSubTab)?.label || activeSubTab}</span>
          </div>
        </div>
