...
// Strategy: fetch raw wikitext infobox first, parse salary/wage fields.
// If not found, scan the full wikitext for currency + wage context.
async function searchWageOnWikipedia(playerName, position) {
  // Step 1: Search for the player's Wikipedia page
  const searchUrl =
    `https://en.wikipedia.org/w/api.php?action=query&list=search` +
    `&srsearch=${encodeURIComponent(playerName + " footballer")}` +
  ...
  if (!hits || hits.length === 0) throw new Error("Player not found on Wikipedia");

  // Pick the best hit — prefer one whose title contains the player's first or last name
  const nameParts = playerName.toLowerCase().split(" ");
  const bestHit =
    hits.find(h => nameParts.some(part => h.title.toLowerCase().includes(part))) || hits[0];

  const pageId = bestHit.pageid;

  // Step 2: Fetch the raw wikitext (infobox lives here)
  const wikitextUrl =
    `https://en.wikipedia.org/w/api.php?action=query&prop=revisions` +
    `&rvprop=content&rvslots=main&pageids=${pageId}` +
    `&format=json&origin=*`;

  const wikitextRes = await fetch(wikitextUrl);
  if (!wikitextRes.ok) throw new Error("Failed to fetch Wikipedia article");
  const wikitextData = await wikitextRes.json();
  const wikitext =
    wikitextData?.query?.pages?.[pageId]?.revisions?.[0]?.slots?.main?.["*"] ||
    wikitextData?.query?.pages?.[pageId]?.revisions?.[0]?.["*"] ||
    "";

  if (!wikitext) throw new Error("Wikipedia article content is empty");

  // Step 3: Parse infobox structured fields first
  // Infobox fields look like: | salary = £150,000 per week
  const infoboxFieldPatterns = [
    /\|\s*(?:weekly[_\s]?wage|salary|wage|earnings?|pay)\s*=\s*([^\n\|<\[]+)/gi,
  ];

  for (const pattern of infoboxFieldPatterns) {
    const match = pattern.exec(wikitext);
    if (match) {
      const raw = match[1].trim().replace(/<!--.*?-->/g, "").replace(/\[\[|\]\]/g, "").trim();
      if (raw && raw.length > 0 && raw.length < 80) return raw;
    }
  }

  // Step 4: Scan full wikitext for currency amounts near wage/salary context
  // Look for patterns like £300,000, €200,000, $150,000 within 120 chars of wage keywords
  const wageKeywords = /(?:wage|salary|earns?|paid|earning|weekly|per\s+week)/i;
  const currencyPattern = /[£€$₤]\s?[\d,]+(?:\.\d+)?(?:\s?(?:million|m|k))?/gi;

  // Split into sentences/chunks and look for ones mentioning wages + a currency
  const chunks = wikitext.split(/[.\n]/);
  for (const chunk of chunks) {
    if (wageKeywords.test(chunk) && currencyPattern.test(chunk)) {
      const amounts = chunk.match(/[£€$₤]\s?[\d,]+(?:\.\d+)?(?:\s?(?:million|m|k))?(?:\s?(?:per\s+week|a\s+week|\/week|pw|weekly))?/gi);
      if (amounts && amounts.length > 0) {
        return amounts[0].trim() + (
          /per\s+week|a\s+week|\/week|pw|weekly/i.test(chunk) &&
          !/per\s+week|a\s+week|\/week|pw|weekly/i.test(amounts[0])
            ? " per week" : ""
        );
      }
    }
  ...
  // Step 5: Fetch plain text extract as last resort and scan it
  const extractUrl =
    `https://en.wikipedia.org/w/api.php?action=query&prop=extracts` +
    `&exintro=false&explaintext=true&pageids=${pageId}` +
    `&format=json&origin=*`;
  const extractRes = await fetch(extractUrl);
  const extractData = await extractRes.json();
  const extract = extractData?.query?.pages?.[pageId]?.extract || "";

  const extractChunks = extract.split(/[.\n]/);
  for (const chunk of extractChunks) {
    if (wageKeywords.test(chunk)) {
      const amounts = chunk.match(/[£€$₤]\s?[\d,]+(?:\.\d+)?(?:\s?(?:million|m|k))?(?:\s?(?:per\s+week|a\s+week|\/week|pw|weekly))?/gi);
      if (amounts && amounts.length > 0) {
        return amounts[0].trim() + (
          /per\s+week|a\s+week|\/week|pw|weekly/i.test(chunk) &&
  ...
  throw new Error("Wage data not found on Wikipedia for this player. Enter manually.");
}
...
function PlayerSlotPopup({ slotIndex, role, existingPlayer, allPlayers, teamPath, team, isAdmin, onClose }) {
  ...
  async function handleWikiWageSearch() {
    ...
    setWageFound(false);
    try {
      const result = await searchWageOnWikipedia(name.trim(), position);
      setWage(result);
      setWageFound(true);
    } catch (e) {
