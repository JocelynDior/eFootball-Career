  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "absolute", top: 0, left: 0 }}>
      <div style={{
        ...
        {current.iconUrl
          ? <img src={current.iconUrl} alt={current.name || ""} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          : <div style={{ width: "100%", height: "100%", background: "rgba(255,20,147,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem" }}>⚽</div>
        }
      </div>
    </div>
  );
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {tournament.description && <div style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.72rem", marginTop: "0.2rem" }}>{tournament.description}</div>}
        </div>
        {fixtures.length > 0 && (
      ...
      {open && fixtures.length > 0 && (
        <div style={{ marginTop: "0.7rem", background: "rgba(0,0,0,0.65)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "0.8rem", padding: "0.7rem" }}>
          {fixtures.map((f, i) => {
            ...
            return (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 36px minmax(0,1fr)", alignItems: "center", gap: "6px", padding: "0.5rem 0", borderBottom: i < fixtures.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
          ...
          .map(({ year, month }, mi) => {
            ...
            return (
              <div key={`${year}-${month}`} style={{ ...GLASS, borderRadius: "1.8rem", overflow: "hidden", marginBottom: "2.5rem", boxShadow: "0 20px 60px rgba(0,0,0,0.4), 0 0 30px rgba(255,255,255,0.03)", animation: `cardIn 0.65s ${mi * 0.07}s both` }}>
                {/* Month header */}
                <div style={{ padding: "1rem 1.6rem", background: "rgba(0,0,0,0.98)", borderBottom: "2px solid rgba(255,255,255,0.15)", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
              ...
              {(selectedEv.eventPairs || []).length === 0
                ...
                : (selectedEv.eventPairs || []).map((pair, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.6rem", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.8rem", padding: "0.7rem", marginBottom: "0.6rem" }}>
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.6rem", color: "#fff", fontWeight: 700, whiteSpace: "nowrap", marginRight: "0.5rem" }}>EVENT {i + 1}</div>
                    {pair.iconUrl && <img src={pair.iconUrl} alt="" style={{ width: "38px", height: "38px", objectFit: "contain", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.2)", flexShrink: 0, background: "rgba(0,0,0,0.5)" }} />}
