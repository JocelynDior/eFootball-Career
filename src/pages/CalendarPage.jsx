...
// Section label — 2x bigger for popup
function SectionLabel({ children }) {
  return (
    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.2rem", fontWeight: 700, color: "#fff", letterSpacing: "0.15em", margin: "1.4rem 0 0.8rem", display: "flex", alignItems: "center", gap: "0.5rem", }}>
      {children}
...
// Flip inner — unchanged from last working version
  ...
  return (
    <div className="flip-inner" ref={innerRef}>
      <div className="flip-face">
        {initial.iconUrl ? <img ref={imgRef} src={initial.iconUrl} alt={initial.name || ""} /> : <div style={{ width: "100%", height: "100%", background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem" }}>⚽</div> }
      </div>
    </div>
  );
}

// Tournament card in popup — all text 2x bigger
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
        {tournament.iconUrl && <img src={tournament.iconUrl} alt="" style={{ width: "60px", height: "60px", objectFit: "contain", borderRadius: "8px", flexShrink: 0 }} />}
        <div style={{ flex: 1 }}>
        ...
        {fixtures.length > 0 && (
          <button onClick={() => setOpen(o => !o)} style={{ background: "none", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "0.35rem 0.85rem", borderRadius: "1.5rem", fontSize: "0.85rem", fontFamily: "inherit", fontWeight: 700, cursor: "pointer", letterSpacing: "0.06em", whiteSpace: "nowrap", transition: "all 0.2s" }}>
      ...
      {open && fixtures.length > 0 && (
        <div style={{ marginTop: "0.7rem", background: "rgba(0,0,0,0.65)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "0.8rem", padding: "0.7rem" }}>
          {fixtures.map((f, i) => {
            ...
            return (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 60px minmax(0,1fr)", alignItems: "center", gap: "8px", padding: "0.7rem 0", borderBottom: i < fixtures.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", minWidth: 0 }}>
                  {homeIcon && <img src={homeIcon} alt="" style={{ width: "42px", height: "42px", objectFit: "contain", borderRadius: "6px", flexShrink: 0 }} />}
                  <span style={{ fontSize: "1.56rem", fontWeight: 600, color: "#fff", wordBreak: "break-word", lineHeight: 1.3 }}>{f.home}</span>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", flexDirection: "row-reverse", minWidth: 0 }}>
                  {awayIcon && <img src={awayIcon} alt="" style={{ width: "42px", height: "42px", objectFit: "contain", borderRadius: "6px", flexShrink: 0 }} />}
                  <span style={{ fontSize: "1.56rem", fontWeight: 600, color: "#fff", wordBreak: "break-word", lineHeight: 1.3, textAlign: "right" }}>{f.away}</span>
                </div>
              </div>
            );
          })}
...
export default function CalendarPage() {
  ...
  // Global flip interval — mirrors HTML exactly
  useEffect(() => {
    ...
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ padding: "24px 20px 60px" }}>
        ...
        <div style={{ marginBottom: "28px", display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "2.8rem", letterSpacing: "6px", color: "#fff", margin: 0 }}>CALENDAR</h1>
              ...
              <div key={`${year}-${month}`} style={{ ...GLASS, borderRadius: "1.8rem", overflow: "hidden", marginBottom: "2.5rem", boxShadow: "0 20px 60px rgba(0,0,0,0.4)", animation: `cardIn 0.65s ${mi * 0.07}s both` }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
                  ...
                  {Array.from({ length: dim }, (_, di) => {
                    ...
                    return (
                      <div key={d} onClick={() => { if (hasEvent) openDayModal(ds); }} style={{ position: "relative", border: "1px solid rgba(255,255,255,0.05)", background: "rgba(26,26,26,0.45)", minHeight: "115px", cursor: hasEvent ? "pointer" : "default", transition: "background 0.2s, border-color 0.25s", opacity: selectedTeam && !visibleInFilter ? 0.25 : 1, overflow: "hidden", }} onMouseOver={e => { if (hasEvent) { e.currentTarget.style.background = "rgba(184,150,12,0.3)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; } }} onMouseOut={e => { if (hasEvent) { e.currentTarget.style.background = "rgba(26,26,26,0.45)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.05)"; } }}>
                        {hasEvent && visibleInFilter && (
                          <>
                            {selectedTeam ? (
                              (() => {
                                const oppIcon = getOppIcon(ev, selectedTeam);
                                return oppIcon ? <img src={oppIcon} alt="" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover", zIndex: 0 }} /> : null;
                              })()
                            ) : (
                              pairs.length > 0 && (
                                // Full-bleed flip: perspective wrapper covers the whole cell
                                <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", perspective: "300px", zIndex: 0 }}>
      ...
      {/* ── Day Detail Modal — all text 2x bigger, buttons unchanged ── */}
      <Modal active={dayModalOpen} onClose={() => setDayModalOpen(false)} wide>
        <div>
          {/* Nav arrows */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.2rem" }}>
            <button onClick={() => navigateModal(-1)} disabled={navIdx <= 0} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", width: "34px", height: "34px", borderRadius: "50%", cursor: navIdx <= 0 ? "default" : "pointer", fontSize: "1.1rem", opacity: navIdx <= 0 ? 0.25 : 1, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>◀</button>
            ...
            <button onClick={() => navigateModal(1)} disabled={navIdx >= allDays.length - 1} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", width: "34px", height: "34px", borderRadius: "50%", cursor: navIdx >= allDays.length - 1 ? "default" : "pointer", fontSize: "1.1rem", opacity: navIdx >= allDays.length - 1 ? 0.25 : 1, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>▶</button>
          </div>

          {selectedEv ? (
            <>
              <SectionLabel>EVENTS ON THIS DAY</SectionLabel>
              {(selectedEv.eventPairs || []).length === 0 ? <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "1.7rem", padding: "1rem" }}>No events scheduled for this day.</div> : (selectedEv.eventPairs || []).map((pair, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.8rem", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "0.8rem", padding: "0.9rem", marginBottom: "0.6rem" }}>
                  {/* EVENT label 2x */}
                  ...
                  {pair.iconUrl && <img src={pair.iconUrl} alt="" style={{ width: "56px", height: "56px", objectFit: "contain", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", flexShrink: 0, background: "rgba(0,0,0,0.5)" }} />}
                  {/* Event name 2x */}
                  <span style={{ flex: 1, fontSize: "1.7rem", color: "#fff", wordBreak: "break-word", fontWeight: 600 }}>{pair.name}</span>
                </div>
              ))}
              ...
              <div style={{ marginBottom: "1rem" }}>
                {/* Season value 2x */}
                <div style={{ background: "rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "0.7rem", padding: "0.75rem 0.9rem", color: "#fff", fontSize: "1.8rem", fontWeight: 600 }}>
                  {selectedEv?.season ? `Season ${selectedEv.season}` : "Not specified"}
                </div>
              </div>

              <SectionLabel>TOURNAMENTS</SectionLabel>
              {(selectedEv.tournaments || []).length === 0 ? <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "1.7rem", padding: "1rem" }}>No tournaments for this date.</div> : (selectedEv.tournaments || []).map((t, ti) => (
