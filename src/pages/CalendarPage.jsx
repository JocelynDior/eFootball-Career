...
function TournamentCard({ tournament, getTeamIcon }) {
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.72)", border: "1px solid rgba(255,20,147,0.14)", borderRadius: "1.1rem", padding: "0.9rem 1rem", marginBottom: "0.8rem", transition: "border-color 0.2s" }}
      onMouseOver={e => e.currentTarget.style.borderColor = "rgba(255,20,147,0.28)"}
      onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,20,147,0.14)"}
    >
      <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
        {tournament.iconUrl && <img src={tournament.iconUrl} alt="" style={{ width: "38px", height: "38px", objectFit: "contain", borderRadius: "6px", flexShrink: 0 }} />}
        <div style={{ flex: 1 }}>
          <div style={{ color: "#fff", fontWeight: 700, fontSize: "1rem" }}>{tournament.name || "Tournament"}</div>
          {tournament.description && <div style={{ color: "#FF1493", fontSize: "0.72rem", marginTop: "0.2rem" }}>{tournament.description}</div>}
        </div>
        {fixtures.length > 0 && (
          <button onClick={() => setOpen(o => !o)} style={{ background: "none", border: "1px solid rgba(255,20,147,0.18)", color: "rgba(255,255,255,0.6)", padding: "0.25rem 0.65rem", borderRadius: "1.5rem", fontSize: "0.67rem", fontFamily: "inherit", fontWeight: 700, cursor: "pointer", letterSpacing: "0.06em", whiteSpace: "nowrap", transition: "all 0.2s" }}>
      ...
      {open && fixtures.length > 0 && (
        <div style={{ marginTop: "0.7rem", background: "rgba(0,0,0,0.65)", border: "1px solid rgba(255,20,147,0.07)", borderRadius: "0.8rem", padding: "0.7rem" }}>
          {fixtures.map((f, i) => {
            const homeIcon = getTeamIcon(f.home) || f.homeIcon;
            const awayIcon = getTeamIcon(f.away) || f.awayIcon;
            return (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 36px minmax(0,1fr)", alignItems: "center", gap: "6px", padding: "0.5rem 0", borderBottom: i < fixtures.length - 1 ? "1px solid rgba(255,20,147,0.06)" : "none" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px", minWidth: 0 }}>
                  {homeIcon && <img src={homeIcon} alt="" style={{ width: "26px", height: "26px", objectFit: "contain", borderRadius: "4px", flexShrink: 0 }} />}
                  <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "#fff", wordBreak: "break-word", whiteSpace: "normal", lineHeight: 1.3 }}>{f.home}</span>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "6px", flexDirection: "row-reverse", minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "#fff", wordBreak: "break-word", whiteSpace: "normal", lineHeight: 1.3, textAlign: "right" }}>{f.away}</span>
                </div>
              </div>
            );
          })}
    ...
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'Inter', sans-serif" }}>
          ...
          .map(({ year, month }, mi) => {
            ...
            return (
              <div key={`${year}-${month}`} style={{ ...GLASS, borderRadius: "1.8rem", overflow: "hidden", marginBottom: "2.5rem", boxShadow: "0 20px 60px rgba(0,0,0,0.4), 0 0 30px rgba(255,20,147,0.08)", animation: `cardIn 0.65s ${mi * 0.07}s both` }}>
                {/* Month header */}
                <div style={{ padding: "1rem 1.6rem", background: "rgba(0,0,0,0.98)", borderBottom: "2px solid #FF1493", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          minHeight: "115px", padding: "8px 5px 6px",
                          cursor: hasEvent ? "pointer" : "default",
                          position: "relative", transition: "background 0.2s, border-color 0.25s",
                          opacity: selectedTeam && !visibleInFilter ? 0.25 : 1,
                          overflow: "hidden",
                        }}
                        onMouseOver={e => { if (hasEvent) { e.currentTarget.style.background = "rgba(184,150,12,0.3)"; e.currentTarget.style.borderColor = "rgba(255,20,147,0.22)"; } }}
                        onMouseOut={e => { if (hasEvent) { e.currentTarget.style.background = hasEvent && visibleInFilter ? "rgba(51,51,51,0.58)" : "rgba(26,26,26,0.45)"; e.currentTarget.style.borderColor = "rgba(255,20,147,0.07)"; } }}
                      >
                        {/* Day number */}
                          ...
                          <>
                            {selectedTeam ? (
                              (() => {
                                ...
                                return oppIcon ? <img src={oppIcon} alt="" style={{ width: "48px", height: "48px", objectFit: "contain", borderRadius: "8px", filter: "drop-shadow(0 3px 8px rgba(0,0,0,0.5))" }} /> : null;
                              })()
                            ) : (
                        ...
                        {!hasEvent && (
                          <div style={{ fontSize: "0.58rem", color: "rgba(255,20,147,0.22)", fontStyle: "italic", marginTop: "auto", paddingTop: "4px", transition: "color 0.2s" }}>No event</div>
                        )}
                      </div>
                    );
                  })}
      ...
      <Modal active={dayModalOpen} onClose={() => setDayModalOpen(false)} wide>
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.2rem" }}>
            <button onClick={() => navigateModal(-1)} disabled={navIdx <= 0} style={{ background: "rgba(255,20,147,0.2)", border: "1px solid rgba(255,20,147,0.4)", color: "#FF1493", width: "34px", height: "34px", borderRadius: "50%", cursor: navIdx <= 0 ? "default" : "pointer", fontSize: "1.1rem", opacity: navIdx <= 0 ? 0.25 : 1, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s" }}>◀</button>
            ...
            <button onClick={() => navigateModal(1)} disabled={navIdx >= allDays.length - 1} style={{ background: "rgba(255,20,147,0.2)", border: "1px solid rgba(255,20,147,0.4)", color: "#FF1493", width: "34px", height: "34px", borderRadius: "50%", cursor: navIdx >= allDays.length - 1 ? "default" : "pointer", fontSize: "1.1rem", opacity: navIdx >= allDays.length - 1 ? 0.25 : 1, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s" }}>▶</button>
          </div>
          ...
          {selectedEv ? (
            <>
              ...
              {(selectedEv.eventPairs || []).length === 0
                ? <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", padding: "1rem" }}>No events scheduled for this day.</div>
                : (selectedEv.eventPairs || []).map((pair, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.6rem", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,20,147,0.1)", borderRadius: "0.8rem", padding: "0.7rem", marginBottom: "0.6rem" }}>
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.6rem", color: "#FF1493", fontWeight: 700, whiteSpace: "nowrap", marginRight: "0.5rem" }}>EVENT {i + 1}</div>
                    {pair.iconUrl && <img src={pair.iconUrl} alt="" style={{ width: "38px", height: "38px", objectFit: "contain", borderRadius: "6px", border: "1px solid rgba(255,20,147,0.4)", flexShrink: 0, background: "rgba(0,0,0,0.5)" }} />}
                    <span style={{ flex: 1, fontSize: "0.85rem", color: "#fff", wordBreak: "break-word" }}>{pair.name}</span>
                  </div>
                ))
              }
              ...
              <div style={{ marginBottom: "1rem" }}>
                <input
                  ...
                  style={{ ...inputStyle, opacity: 0.8, cursor: "default", background: "rgba(0,0,0,0.6)", width: "100%", borderRadius: "0.7rem", border: "1px solid rgba(255,20,147,0.4)", color: "#fff", padding: "0.65rem 0.9rem", fontFamily: "inherit", fontSize: "0.9rem", outline: "none", boxSizing: "border-box" }}
                />
              </div>
              ...
              {(selectedEv.tournaments || []).length === 0
                ? <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", padding: "1rem" }}>No tournaments for this date.</div>
          ...
          <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", marginTop: "1.2rem" }}>
            <button onClick={() => setDayModalOpen(false)} style={{ padding: "0.55rem 1.3rem", borderRadius: "2rem", fontFamily: "inherit", fontWeight: 700, fontSize: "0.85rem", cursor: "pointer", background: "rgba(255,20,147,0.45)", border: "1px solid rgba(255,20,147,0.4)", color: "#FF1493", letterSpacing: "0.06em" }}>CLOSE</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal active={filterOpen} onClose={() => setFilterOpen(false)}>
        ...
        <input
          ...
          style={{ width: "100%", background: "rgba(0,0,0,0.7)", border: "1px solid rgba(255,20,147,0.4)", borderRadius: "0.7rem", color: "#fff", padding: "0.55rem 0.9rem", fontSize: "0.85rem", fontFamily: "inherit", outline: "none", marginBottom: "0.8rem", boxSizing: "border-box", transition: "border-color 0.2s" }}
        />
