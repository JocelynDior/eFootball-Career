...
export default function SubmitResultModal({ league, season, teams, onClose }) {
  ...
  const [scorers, setScorers] = useState([]); // { player, goals, imageUrl, imageFile }
  const [assists, setAssists] = useState([]);  // { player, assists, imageUrl, imageFile }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const scorersData = await Promise.all(scorers.map(async s => {
        ...
        if (s.imageFile) {
          try { imageUrl = await uploadToImgBB(s.imageFile); } catch (e) {}
        }
      ...
      const assistsData = await Promise.all(assists.map(async a => {
        ...
        if (a.imageFile) {
          try { imageUrl = await uploadToImgBB(a.imageFile); } catch (e) {}
        }
  ...
  return (
    <div>
      ...
      {status && (
        <div style={{ color: status.startsWith("✅") ? "#22c55e" : "#ff6b6b", fontSize: "0.85rem", margin: "12px 0", textAlign: "center" }}>{status}</div>
      )}

      <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
        <button onClick={handleSubmit} disabled={saving} style={{ flex: 1, padding: 14, background: "#FF1493", border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
