...
export default function AddTeamModal({ league, season, team = null, onClose }) {
  const isEdit = !!team;
  const [clubs, setClubs] = useState([]); // { name, badge }
  const [form, setForm] = useState({
    name: team?.name || "",
    p: team?.p || 0, w: team?.w || 0, d: team?.d || 0,
    l: team?.l || 0, gs: team?.gs || 0, gc: team?.gc || 0,
    gd: team?.gd || 0, pts: team?.pts || 0,
  });
  const [saving, setSaving] = useState(false);
  const [status, setStatus] = useState("");

  // Load all clubs from career_team_management (same path admin uses)
  useEffect(() => {
