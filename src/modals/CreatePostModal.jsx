...
export default function CreatePostModal({ post = null, onClose }) {
    ...
    for (const file of files) {
      try {
        ...
        if (url) {
          ...
          setStatus("One image failed to upload, skipping.");
        }
      } catch (err) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const validMedia = mediaItems.filter(m => m && m.url);
  ...
  return (
    <div>
      <h3 style={{ color: "#FF1493", fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.8rem", marginBottom: "20px" }}>
        {isEdit ? "✏️ Edit Post" : "📸 Create Post"}
      </h3>
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          {mediaItems.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", background: "rgba(255,20,147,0.08)", borderRadius: "10px", marginBottom: "6px" }}>
              <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.85rem", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {item.type === "video" ? "🎥" : "🖼️"} {item.url?.substring(0, 40)}...
              </span>
      ...
      <div style={{ display: "flex", gap: "12px", marginTop: "8px" }}>
        ...
        >
          {saving ? "Saving..." : isEdit ? "Update Post" : "Post"}
        </button>
        <button
          onClick={onClose}
