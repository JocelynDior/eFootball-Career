  ...
  return (
    <div style={{ textAlign: "center", padding: "24px 16px" }}>
      <div style={{
        ...
        fontSize: "1.6rem", letterSpacing: "4px", marginBottom: "24px",
        ...
        {units.map(({ label, val }) => (
          <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px" }}>
            <div style={{
              ...
              background: "#FF1493",
              boxShadow: "0 0 30px rgba(255,20,147,0.5), inset 0 0 20px rgba(255,255,255,0.1)",
              display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center"
            }}>
              <span style={{ color: "#000033", fontFamily: "'Bebas Neue', sans-serif", fontSize: "2rem", fontWeight: 900, lineHeight: 1 }}>{val}</span>
            </div>
            <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "0.7rem", fontWeight: 700, letterSpacing: "2px" }}>{label}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div onMouseDown={() => setPaused(true)} onMouseUp={() => setPaused(false)}
      ...
      {countdowns.length > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: "8px", paddingBottom: "12px" }}>
          {countdowns.map((_, i) => (
            <span key={i} onClick={() => setIdx(i)} style={{ width: "8px", height: "8px", borderRadius: "50%", background: i === idx ? "#FF1493" : "rgba(255,255,255,0.3)", cursor: "pointer", display: "block", transition: "all 0.3s" }} />
          ))}
        </div>
      )}
    </div>
